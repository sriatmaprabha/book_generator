...
async def run_structurer(
    ...
    agent = Agent(
        ...
        instructions=[
            ...
            "  - chapter_title: a compelling title derived from the transcript's main theme",
            ...
            "  - suggested_additional_refs: 2-3 shastra references that could supplement the teaching",
            "",
            "Also provide:",
            "  - book_title: overall title for the compiled book",
...
async def run_shastra_enrichment(
    ...
    async def enrich_one(ch_plan: ChapterPlan, transcript: TranscriptFile) -> ShastraEnrichment:
        ...
        agent = Agent(
            ...
            instructions=[
                f"Find 3-5 EXACT Sanskrit shastra pramanas for Day {ch_plan.day_number}.",
...
async def run_humor_and_shastras(
    ...
    async def find_more_shastras(ch_plan: ChapterPlan, transcript: TranscriptFile) -> ExtendedShastras:
        ...
        agent = Agent(
            ...
            model=build_agent_model("researcher", config),
            instructions=[
                f"Find 3-5 EXACT Sanskrit shastra pramanas for chapter: {ch_plan.chapter_title}",
    ...
    # Run all humor + shastra tasks in parallel
    all_tasks = []
    for ch_plan, transcript in zip(structure.chapters, transcripts):
    ...
    return humor_results, shastra_results
...
async def run_formatting(
    ...
    Humor stories and extended shastras are INSERTED (additions, not replacements).
    """
    ...
    async def correct_one(chapter: FormattedChapter) -> FormattedChapter:
        ...
        for chunk in chunks:
            ...
            agent = Agent(
                ...
                instructions=[
                    "Fix ONLY grammatical errors in this satsang transcript text.",
                    "Rules: fix subject-verb agreement, tense, articles, punctuation, capitalization.",
                    "DO NOT remove, add, reorder, rephrase, or condense ANY content.",
                    "DO NOT change Swamiji's speaking style — 'Listen.', fragments are INTENTIONAL.",
                    "DO NOT alter Sanskrit terms, mantras, markdown formatting (##, ---, > *).",
                    "Remove stray backslashes. Use round braces () not square brackets [].",
...
async def compile_satsang_book(
    ...
    try:
        ...
        formatted = await run_formatting(
            transcripts, structure, enrichments, tracer, app_config,
